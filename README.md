...
State is local by default. `terraform/backend.tf.example` shows an S3 backend — rename it
to `backend.tf`, fill in your own bucket, and run `terraform init -migrate-state`. The
bucket and its locking are provisioned separately; this stack does not create them.
...
validates it with headroom instead of just touching the target.

Three thresholds:

| Threshold | What it catches |
| --- | --- |
| `server_errors < 0.1%` | 5xx responses, matching the SLO definition |
| `http_req_failed < 0.1%` | Stricter — k6 counts any non-2xx/3xx, so this also trips on 4xx |
| `http_req_duration p(99.9) < 300 ms` | The latency SLO |

`server_errors` is a custom rate in the script, added because k6's built-in
`http_req_failed` does not distinguish client errors from server errors. It is measured
from the client, so it is a close proxy for the CloudWatch server-side success rate
rather than the same measurement — the ALB sees requests that never reach a client, and
vice versa. Every request in this synthetic workload should be a 200, so
`http_req_failed` is a useful canary even though it is stricter than the SLO.
...
- Terraform state is local unless you add the backend described above.
- No VPC Flow Logs. Useful for network forensics, but extra resources and cost for a
  workload this simple.
